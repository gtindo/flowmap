...
Tests are indexed but hidden until the **Tests** toggle is enabled. A mixed repository exposes a language picker so Go and JavaScript/TypeScript remain separate graphs. JavaScript support covers `.js`, `.mjs`, `.cjs`, `.jsx`, `.ts`, `.mts`, `.cts`, and `.tsx` without requiring Node.js; it follows only relative local imports. Constructors and class methods are indexed as owner-qualified callables, while local `this`, `super`, and static calls resolve directly. Calls through locally constructed or explicitly typed local instances are dotted dynamic dependencies; unknown or computed receivers remain unresolved. Anonymous functions appear when traversing their named parents but stay out of search and Git change lists. Non-local packages remain outside the visible graph. Dashed edges are interface or dynamic-dispatch candidates, while dotted edges show local functions passed as arguments or conservative JavaScript receiver relationships.
