...
func TestHandlerServesNavigableGraphViews(t *testing.T) {
	...
	for _, expected := range []string{"--surface:", "--context-bar-height: 40px", ":root[data-theme=\"dark\"]", ".app-header", ".control-group", ".context-controls", ".context-controls .control-group { min-height: 30px", "margin-left: auto", ".git-review", ".changes-menu", ".change-item", ".reviewed-list-badge", ".reviewed-node-badge", ".reviewed-action", "--reviewed:", ".diff-addition", ".diff-deletion", ".node .name.new { fill: var(--change-new); }", ".node .name.updated { fill: var(--change-updated); }", "#edges path.dependency", "stroke-dasharray: 2 5", "#canvas-wrap", "overflow: auto", "#detail", "top: var(--context-bar-height)", "height: calc(100% - var(--context-bar-height))", "width: var(--detail-width", "@media (max-width: 1480px)", "grid-template-columns: auto minmax(300px, 1fr)", "grid-template-areas: \"brand search\" \"controls controls\""} {
